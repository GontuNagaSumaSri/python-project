...
def add_event(request):
    if not request.user.is_authenticated:
        return redirect('Login')
    error = ""
    category = IndustryPractices.objects.all()
    if request.method=='POST':
        ...
        try:
           Job.objects.create(eventname=en,image=i,category=cat,description=des,startdate=sd,enddate=ed,venue=ve,entryprice=ep)
           error="no"
        except:
           error="yes"
...
def add_sponsor(request):
    if not request.user.is_authenticated:
        return redirect('Login')
    error = ""
    event1=Job.objects.all()
    if request.method=='POST':
        eventid=request.POST['event']
        i=request.FILES['image']
        event = Job.objects.get(id = eventid)
        try:
...
def add_category(request):
    ...
    if request.method=='POST':
        ...
        try:
           IndustryPractices.objects.create(categoryname=cn)
           error="no"
        except:
            error="yes"
    d = {'error':error}
...
def view_category(request):
    if not request.user.is_authenticated:
        return redirect('Login')
    cat = IndustryPractices.objects.all()
    d = {'cat':cat}
...
def delete_category(request,pid):
      if not request.user.is_authenticated:
        return redirect('Login')
      apoint= IndustryPractices.objects.get(id=pid)
      apoint.delete()
...
def innerhome(request):
    if not request.user.is_authenticated:
        return redirect('Login')
    totalevents = Job.objects.all().count()
    totalsponsor = SponsorTbl.objects.all().count()
    totalcategory = IndustryPractices.objects.all().count()
...
def edit_category(request,pid):
    if not request.user.is_authenticated:
        return redirect('Login')
    error = ""
    category = IndustryPractices.objects.get(id=pid)
    if request.method=='POST':
        cn=request.POST['cname']
        try:
